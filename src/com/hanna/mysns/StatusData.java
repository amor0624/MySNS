...
private static final String TAG = StatusData.class.getSimpleName();
	...
	class DbHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("create table " + TWITTERTABLE + " (" + C_ID
					+ " int primary key, " + C_CREATED_AT + " int, " + C_USER_ID
					+ " INTEGER,"  +C_USER_NAME
					+ " text,"  +C_USER_IMAGEURL
					+ " text," + C_TEXT + " text)");
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			db.execSQL("drop table if exists " + TWITTERTABLE); // drops the old database
    ...
    public void insertOrIgnore(ContentValues values) {
		...
		try {
			db.insertWithOnConflict(TWITTERTABLE, null, values, SQLiteDatabase.CONFLICT_IGNORE);
		} finally {
			db.close();
		}	
	}
	...
	public Cursor getStatusUpdates() {
		...
		return db.query(TWITTERTABLE, null, null, null, null, null, GET_ALL_ORDER_BY_STRING);
	}
	...
	public  long getLatestStatusCreatedAtTime() {
		SQLiteDatabase db = this.dbHelper.getReadableDatabase();
		try {
			Cursor cursor = db.query(TWITTERTABLE, MAX_CREATED_AT_COLUMNS, null, null, null, null, null);
			try {
	...
	public String getStatusTextById(long id) {
		SQLiteDatabase db = this.dbHelper.getReadableDatabase();
		try {
			Cursor cursor = db.query(TWITTERTABLE, DB_TEXT_COLUMNS, C_ID+"="+id, null,null, null, null);
			try{
